...
def project_world_objects_corner(object_dict,
                          output_dict,
                          lidar_pose,
                          lidar_range,
                          order):
    ...
    for object_id, object_content in object_dict.items():
        ...
        bbx_lidar = mask_boxes_outside_range_numpy(bbx_lidar,
                                                   lidar_range,
                                                   order)
        bbx_lidar = boxes_to_corners_3d(bbx_lidar,order)
        if bbx_lidar.shape[0] > 0:
            output_dict.update({object_id: bbx_lidar})
...
def get_points_outside_rotated_box_3d(p, box_corner):
    """
    Get points outside a rotated bounding box (3D version).
    ...
    # A point is outside the box if any of its projections onto the three edges
    # is less than 0 or greater than 1.
    mask1 = np.logical_or(l1 < 0, l1 > 1)
    mask2 = np.logical_or(l2 < 0, l2 > 1)
    mask3 = np.logical_or(l3 < 0, l3 > 1)

    # Combine the masks to find points outside the box
    outside_mask = np.logical_or(mask1, mask2)
    outside_mask = np.logical_or(outside_mask, mask3)

    # Select points outside the box using the mask
    p_outside_box = p[outside_mask]

    return p_outside_box


def get_points_inside_box(corner_points, points):
    """
    Get points inside the box defined by corner points.

    Parameters
    ----------
    corner_points : np.ndarray
        Numpy array of corner points with shape (3, 8), where each column represents a corner point (x, y, z).
    points : np.ndarray
        Numpy array of points with shape (N, 3), where N is the number of points.

    Returns
    -------
    inside_points : np.ndarray
        Numpy array of points inside the box.
    """

    # Get min and max coordinates of the box
    min_xyz = np.min(corner_points, axis=1)
    max_xyz = np.max(corner_points, axis=1)

    # Check if points are inside the box
    inside_mask = np.all((min_xyz <= points) & (points <= max_xyz), axis=1)

    inside_points = points[inside_mask]

    return inside_points

def is_point_inside_any_box(points, corner_points):
    # Compute min and max coordinates of each bounding box
    min_corner = np.min(corner_points, axis=1)
    max_corner = np.max(corner_points, axis=1)

    # Check if points are inside any bounding box
    inside_mask = np.all((points >= min_corner) & (points <= max_corner), axis=2)
