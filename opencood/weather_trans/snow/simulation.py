...
import opencood.weather_trans.snow.geometry as g
    ...
    for j, beam_dict in enumerate(occlusion_list):
        ...
        if len(beam_dict.keys()) > 1:                           # otherwise there is no snowflake in the current beam
            ...
            for key, tuple_value in beam_dict.items():
                ...
                end_index = int(np.floor((r_j + c * tau_h) * intervals_per_meter) )
...
def augment(pc: np.ndarray, particle_file_prefix: str, beam_divergence: float, shuffle: bool = True,
    ...
    sensor_info = '/home/gaojing/zjy/adv_14/opencood/weather_trans/snow/calib/20171102_64E_S3.yaml'
    ...
    #         result = process_single_channel(root_path, particle_file_prefix, orig_pc, beam_divergence, order, channel_infos, channel)
...
#
#     path = r'C:\Users\86187\Desktop\2021_09_03_09_32_17\302\006220.pcd'
...
#
#
#     print(augmented_pointcloud.shape)


def apply_snow_to_numpy(points_np):
    mode = 'gunn'
    beam_divergence = 0.003  # (rad)
    snowfall_rate = 0.5 
    ...
    stats, augmented_pointcloud = augment(points_np, only_camera_fov=False,
                                          particle_file_prefix=snowflake_file_prefix,
                                          noise_floor=0.7,
                                          beam_divergence=float(np.degrees(beam_divergence)),
                                          shuffle=True,
