...
INTEGRAL_PATH = Path(os.path.dirname(os.path.realpath(__file__))) / inte_folder/'original'
...
def get_available_alphas() -> List[float]:
    ...
    # alphas = [0.0,0.01,0.02,0.03,0.005,0.06]
...
class ParameterSet:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.beta = 0.01  #0.008  #0.046 / self.mor
...
def apply_fog_to_numpy(point_cloud_np):

    # INTEGRAL_PATH = Path(os.path.dirname(os.path.realpath(__file__))) / 'original'
    ...
    parameter_set = ParameterSet(alpha=available_alpha, gamma=0.000001, beta=0.008)
    noise = 10
    gain = False
    noise_variant = 'v1'
    hard = True
    ...
    augmented_pc, simulated_fog_pc, num_fog, info_dict = simulate_fog(parameter_set, point_cloud_np, noise, gain, noise_variant, hard, soft)
    return augmented_pc
